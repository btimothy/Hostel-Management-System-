...
$query="insert into  registration(roomno,seater,feespm,foodstatus,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,pmntAddress,pmntCity) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisissssssisississss',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$paddress,$pcity);
$stmt->execute();
echo"<script>alert('You Have Succssfully Booked a Room');</script>";
}
?>
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
			<h4 style="color: red" align="center">Hostel already booked by you! Please make your Payment using stablic bank on A/C : 03450000012  <br> MM :0752378171</h4>
				<?php }
				else{
							echo "";
							}			
...
<input type="radio" value="1" name="foodstatus"> With Food(Rs 200000.00 Per Semester Extra)
</div>
</div>	
...
<option value="">Select Duration in Semester</option>
...
<!-- <div class="form-group">
<label class="col-sm-2 control-label">Deposit Amount/All</label>
<div class="col-sm-8">
...
<?php	
$aid=$_SESSION['id'];
	$ret="select * from students where id=?";
